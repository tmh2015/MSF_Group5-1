...
library(stargazer)
library (dplyr)
>>>>>>> a2d4b6e1f054458758291203e70bbb60f15f7987
```
...
## Social Media
This part is to analyse whether people who used social media sites such as Facebook, Twitter or LinkedIn to look for a job, to apply for a job that you found out about on social media or to let their friends know about a job, tend to use online dating app.
...
pvalue <- chisq.test(tab$date2a, tab$social_media_pref)$p.value

xtable(table(factor(tab$date2a,labels=c("Yes","No")),factor(tab$social_media_pref,labels=c("Used","Not used"))))
...
>>>>>>> a2d4b6e1f054458758291203e70bbb60f15f7987
```
P value is $pvalue$ less than 0.05, so the there is a relationship between using online dating app and using social media to find a job. The logistic model 
